...
phone_regex = re.compile(r'''(
	...
	(\s|-|\.)?            # seperator
	(\d{4})               # Last 4 digit
	)''', re.VERBOSE) # re.VERBOSE or re.X allows to separate logical sections instead of doing a whole row 

email_regex = re.compile(r'''(
    [a-zA-Z0-9._%+-]+   #Username
    @                   #  symbol
    [a-zA-Z0-9._%+-]+   # domanin name
    (\.[a-z{2,4}])      # dot-something
    )''', re.VERBOSE)   # allows for separate logical sections

match = []  # list for storing matches found
